...
# Sostituito upload_documnets con upload_documents


def upload_documents(auth, dataset_id, files=None):
...
class TestAuthorization:
    ...
    def test_invalid_auth(self, auth, expected_code, expected_message):
        res = upload_documents(auth, "dataset_id")
...
class TestDocumentsUpload:
    ...
    def test_valid_single_upload(self, get_http_api_auth, add_dataset_func, tmp_path):
        ...
        res = upload_documents(get_http_api_auth, dataset_id, [fp])
        assert res["code"] == 0
    ...
    def test_missing_file(self, get_http_api_auth, add_dataset_func):
        dataset_id = add_dataset_func
        res = upload_documents(get_http_api_auth, dataset_id)
        assert res["code"] == 101
    ...
    def test_invalid_dataset_id(self, get_http_api_auth, tmp_path):
        fp = create_txt_file(tmp_path / "ragflow_test.txt")
        res = upload_documents(get_http_api_auth, "invalid_dataset_id", [fp])
        assert res["code"] == 100
    ...
    def test_duplicate_files(self, get_http_api_auth, add_dataset_func, tmp_path):
        dataset_id = add_dataset_func
        fp = create_txt_file(tmp_path / "ragflow_test.txt")
        res = upload_documents(get_http_api_auth, dataset_id, [fp, fp])
        assert res["code"] == 0
    ...
    def test_multiple_files(self, get_http_api_auth, add_dataset_func, tmp_path):
        ...
        for i in range(expected_document_count):
            ...
            fps.append(fp)
        res = upload_documents(get_http_api_auth, dataset_id, fps)
        assert res["code"] == 0
    ...
    def test_concurrent_upload(self, get_http_api_auth, add_dataset_func, tmp_path):
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [executor.submit(upload_documents, get_http_api_auth, dataset_id, fps[i : i + 1]) for i in range(expected_document_count)]
