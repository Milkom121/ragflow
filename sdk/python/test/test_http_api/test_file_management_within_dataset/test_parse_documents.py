...
import pytest
from common import INVALID_API_TOKEN, bulk_upload_documents, list_documents, parse_docs
...
class TestAuthorization:
    ...
    def test_invalid_auth(self, auth, expected_code, expected_message):
        res = parse_docs(auth, "dataset_id")
...
class TestDocumentsParse:
    ...
    def test_basic_scenarios(self, get_http_api_auth, add_documents_func, payload, expected_code, expected_message):
        ...
        res = parse_docs(get_http_api_auth, dataset_id, payload)
        assert res["code"] == expected_code
        if expected_code != 0:
            assert res["message"] == expected_message
        if expected_code == 0:
    ...
    def test_invalid_dataset_id(
        ...
        res = parse_docs(get_http_api_auth, dataset_id, {"document_ids": document_ids})
        assert res["code"] == expected_code
        assert res["message"] == expected_message
    ...
    def test_duplicate_parse(self, get_http_api_auth, add_documents_func):
        dataset_id, document_ids = add_documents_func
        res = parse_docs(get_http_api_auth, dataset_id, {"document_ids": document_ids + document_ids})
        assert res["code"] == 0
